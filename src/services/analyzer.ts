...
export class AnalyzerService {
   ...
   */
  async analyzeDatabases(client: MongoClient, databaseNames?: string[]): Promise<DatabaseStats[]> {
    try {
      ...
      logger.info(`Analyzing ${databasesToAnalyze.length} database(s): ${databasesToAnalyze.join(', ')}`);
      ...
      for (const dbName of databasesToAnalyze) {
        ...
        for (const collection of collections) {
          ...
          logger.debug(`Analyzed collection ${dbName}.${collName}: ${count} documents, ${stats.size} bytes`);
        }
        ...
        logger.info(`Database ${dbName}: ${collections.length} collections, ${totalDocuments} documents, ${totalSize} bytes`);
      }
   ...
   */
  async validateCompatibility(sourceClient: MongoClient, targetClient: MongoClient): Promise<{ compatible: boolean, issues: string[] }> {
    const issues: string[] = [];
    ...
    try {
      ...
      logger.info(`Source MongoDB version: ${sourceInfo.version}`);
      logger.info(`Target MongoDB version: ${targetInfo.version}`);
      ...
      if (targetVersion[0] === sourceVersion[0] && targetVersion[1] < sourceVersion[1]) {
        logger.warn(`Target MongoDB minor version (${targetInfo.version}) is older than source (${sourceInfo.version}).`);
      }
      ...
      logger.error('Error validating compatibility:', error);
